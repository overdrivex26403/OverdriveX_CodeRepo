...
@TeleOp(name = "SundayTeamTeleop", group = "Robot")

public class SundayTeamTeleop extends LinearOpMode {
    ...
    private DcMotorEx arm   = null; // arm          motor
    ...
    private static final double ARM_COLLAPSED            = 1;
    ...
    // Constants for claw servo positions
    ...
    private              double clawPosition             = CLAW_CLOSED;
    ...
    // Gamepad 2 is only used for SCORING - arm, wrist, claw control 
    private void controlScoring() {
        if (gamepad2.b) {                    // Gamepad 2's A button starts claw rotation
            clawPosition = CLAW_CLOSED;
        } else if (gamepad2.a) {             // Gamepad 2's B button reverses claw rotation
